...
def test_zscore_factory_basic(tiny_cube: xr.DataArray) -> None:
    op = zscore(dim="time", std_eps=STD_EPS)
    z = op(tiny_cube)

    assert z.name.endswith("_zscore")
    assert z.dims == tiny_cube.dims


def test_zscore_missing_dim_raises(tiny_cube: xr.DataArray) -> None:
    op = zscore(dim="not_present")
